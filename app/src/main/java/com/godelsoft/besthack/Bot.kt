...
data class Bot(val botSender: User, val sender: User, val recycleAdapter: MessageAdapter, val connectToSupport: (user: User) -> Unit = {}) {
    ...
    init {
        addMessage("back", "Назад", null, arrayOf("FAQ", "request", "call support"))
            ...
            addMessage("other", "Проблемма другого рода, свяжите меня с оператором", "Соединаем с опретором...", arrayOf(), connectToSupport)
                addMessage("poweroff", "Я не знаю как выключить компьютер", "Чтобы выключить компьютер под управлением OS Windows" +
                        "нажмите на меню пуск в правом нижнем углу, выбере пункт `выключение` -> `Завершение работы`\nВам помогла данная инструкция?", arrayOf("yes", "no"))
                addMessage("notepad", "Болкнот завис", "Для решения проблемы последовательно выполните следующие пункты:" +
            ...
            addMessage("soft", "Проблемма с программой", "Что у вас не работает?", arrayOf("win", "notepad", "excel", "poweroff", "back_FAQ"))
            ...
            addMessage("hard", "Проблемма с комплектующими", "Что у вас не работает?", arrayOf("mouse", "monitor", "back_FAQ"))
        addMessage("FAQ", "Популярные вопросы", "С какого рода проблемой вы столкнулись?", arrayOf("hard", "soft", "other", "back"))
            ...
            addMessage("crash", "Оборудование сломалось", "Перенаправляем в форму заявки", arrayOf("back"))
